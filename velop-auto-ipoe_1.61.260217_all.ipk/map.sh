...
proto_map_setup() {
	...
	local type mtu ttl tunlink zone encaplimit draft03
	local rule ipaddr ip4prefixlen ip6prefix ip6prefixlen peeraddr ealen psidlen psid offset fmr
	json_get_vars type mtu ttl tunlink zone fmr encaplimit draft03
	...
	if [ -z "$rule" ]; then
		...
		# Store BMR as the first argument
		all_rules="$rule"
		
		# Get FMR rules from UCI
		local fmr_list=""
		fmr_list=$(uci -q get network.${cfg}.rule)

		# Process each FMR rule
		for fmr in $fmr_list; do
			# Split FMR entry by comma
			local ipv6prefix=$(echo $fmr | cut -d',' -f1)
			local ipv4prefix=$(echo $fmr | cut -d',' -f2)
			local fmr_ealen=$(echo $fmr | cut -d',' -f3)
			local fmr_offset=$(echo $fmr | cut -d',' -f4)
			
			# Extract prefix lengths
			local prefix6len=$(echo $ipv6prefix | cut -d'/' -f2)
			local prefix4len=$(echo $ipv4prefix | cut -d'/' -f2)
			# Remove CIDR notation from prefixes
			ipv6prefix=$(echo $ipv6prefix | cut -d'/' -f1)
			ipv4prefix=$(echo $ipv4prefix | cut -d'/' -f1)
			
			# Add FMR rule without quotes
			fmr_rule="type=$type,ipv6prefix=$ipv6prefix,prefix6len=$prefix6len,ipv4prefix=$ipv4prefix,prefix4len=$prefix4len,ealen=$fmr_ealen,offset=$fmr_offset,psidlen=$psidlen,fmr=1"
			all_rules="$all_rules $fmr_rule"
		done
	fi

	RULE_DATA=$(mapcalc ${tunlink:-\*} $all_rules)
	if [ "$?" != 0 ]; then
...
# -------------------------------------------------------------------------------------------------
# Firewall mod for Velop
# -------------------------------------------------------------------------------------------------

	    network_get_device ifname "lan"

		# Get gaming configuration
		gaming_enabled=$(uci get ca_setup.getmap.gaming_port 2>/dev/null)
		gaming_ip=$(uci get ca_setup.getmap.gaming_ip 2>/dev/null)

        if [ "$gaming_enabled" = "1" ] && [ -n "$gaming_ip" ]; then

            # Gaming port logic - take first two portsets for gaming device
            portset_count=0
            for portset in $(eval "echo \$RULE_${k}_PORTSETS"); do
                portset_count=$((portset_count + 1))
                
                if [ $portset_count -le 2 ]; then
                    # First two portsets reserved for gaming device traffic
                    for proto in icmp tcp udp; do
                        ...
                        json_add_string src_ip "$gaming_ip"
                    ...
                    # Remaining portsets for other devices (excluding gaming_ip)
                    for proto in icmp tcp udp; do
                        json_add_object ""
                        json_add_string type nat
                        json_add_string target SNAT
			...
			# Default logic - without dedicated gaming portsets
			for portset in $(eval "echo \$RULE_${k}_PORTSETS"); do
				for proto in icmp tcp udp; do
					json_add_object ""
					json_add_string type nat
